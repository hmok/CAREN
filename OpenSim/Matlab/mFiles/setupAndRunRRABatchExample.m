...
ik_results_folder = strcat(destination,'IKResults\');
GRF_results_folder = strcat(destination,'grfResults\');
results_folder = strcat(destination,'RRAResults\');
setupfiles_folder = strcat(destination,'RRASetup\');
% genericSetupForIK = 'IK_HBM_Setup.xml';
copyfile(source, setupfiles_folder)
modelFile = 'subject01_simbody.osim';
model_folder = strcat(destination,'ScaledModel\');
