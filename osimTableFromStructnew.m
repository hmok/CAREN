...
% Optional input: 'table_labels',tabelLabels
%   Where tableLabels is a nX1 cell in the same order as the structure 
%   field names that contains strings that will be used as the labels of 
%   the TimeSeriesTable. This is helpful when table_labels need to be 
%   things like '/path/to/component' that cannot be used as structure
%   names.
...
%    Ayman Habib, Jen Hicks,Shrinidhi K. Lakshmikanth, Colin Smith

function timeseriesosimtable = osimTableFromStruct(s,varargin)

%% Parse Optional Inputs
table_labels = fieldnames(s);

for i = 1:length(varargin)
    if(strcmp(varargin{i},'table_labels'))
        table_labels = varargin{i+1};
    end
end
